    ...
    await Promise.all(
      docs.map(async (doc) => {
        ...
        if (embeddingResults.length !== 1) {
          throw new Error(
            `Expected exactly one embedding result, got ${embeddingResults.length}`
          );
        }
  ...
  async (employeeData: any) => {
    ...
    const documents = chunks.map((text: string) => {
      return Document.fromText(text, {
        ...
        uploadDate: employeeData.uploadDate,
      });
    });
...
export const searchCVFlow = ai.defineFlow(
  ...
  async (query: string) => {
    ...
    docs.forEach((doc) => {
      ...
      if (
        employee?.email &&
        !Array.from(employees).some((e) => e.email === employee?.email)
      ) {
        employees.add(doc.metadata);
      }
    });
    logger.info(
      `Unique employee emails: ${Array.from(employees)
        .map((e) => e.email)
        .join(", ")}`
    );
    ...
    for (const employee of employees) {
      ...
      const matchingDocsText = matchingDocsArray.map((doc) =>
        Document.fromText(doc.text, {
          name: doc.metadata?.name,
          email: doc.metadata?.email,
          fileName: doc.metadata?.fileName,
          uploadDate: doc.metadata?.uploadDate,
